...
(function($) {
    ...
    function doDetail(object) {
        var tAddress, tInhoud, tVraagprijs, tWoonoppervlakte, tOverigeInpandigeRuimte, tGebouwgebondenBuitenruimte, tExterneBergruimte, tEigendomssituatie;

        var elements = object.find('.object-kenmerken-list > *, .object-kenmerken-group-list > dl > *');
        tAddress = object.find('.object-header__address').text();
    ...
    function createDetail(infoObject, element) {
        ...
        for (var key in flattenInfoObject) {
            if (!flattenInfoObject.hasOwnProperty(key)) continue;
            eExtraInfoRowHeader.append($('<th>').text(key.split('.').pop()));
            var value = flattenInfoObject[key];
            eExtraInfoRowData.append($('<td>').text(value));
        }

        element.prepend('<p><strong>pricePerSquareMeters: </strong>'+pricePerSquareMeters+'</p>');
        element.prepend('<p><strong>pricePerSquareMetersExtra: </strong>'+pricePerSquareMetersExtra+'</p>');
        element.prepend('<p><strong>pricePerQubicMeters: </strong>'+pricePerQubicMeters+'</p>');
    ...
    function flatten(input, reference, output) {
        output = output || {};
        for (var key in input) {     if (!input.hasOwnProperty(key)) continue;
                                var value = input[key];
    ...
    $('head').append('<style type="text/css">p,td,tr,th,table{border:1px solid black; text-align:center}</style>');

    var bDetail = $('main div.object-detail').length === 1;
    var bOverview = $('main .container.search-main').length === 1 || $('main .user-saved-objects').length === 1;
    var infoObject = {
        address: '',
        url: '',
        transmission: {
