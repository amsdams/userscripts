...
(function () {
    ...
    var bDetail = $('main div.object-detail')
        .length == 1;
    var bOverview = $('main form')
        .length == 1;
    ...
    if (bOverview) {
        ...
        for (var i = 0; i < objects.length; ++i) {
            ...
            let tAddress = object.find('h3.search-result-title')
                .text();
    ...
    if (bDetail) {
        ...
        let tAddress = object.find('.object-header__address')
            .text();
        ...
        infoObject.surfacesAndContents.useAreas.buildingRelatedOutdoorSpace = parseFloat(tGebouwgebondenBuitenruimte ? tGebouwgebondenBuitenruimte.replace(/[^0-9]/g, "") : 0);
    ...
    function createOverview(infoObject, element) {
        ...
        let eExtraInfo = $("<ul>");
        eExtraInfo.append($("<li>")
            .text('pricePerSquareMeters: ' + pricePerSquareMeters));

        element.find('.search-result-info-price')
            .append(eExtraInfo);
    }

    function createDetail(infoObject, element) {
        let pricePerSquareMeters = infoObject.transmission.price / infoObject.surfacesAndContents.useAreas.livingArea;
        let pricePerSquareMetersExtra = infoObject.transmission.price / (infoObject.surfacesAndContents.useAreas.livingArea + infoObject.surfacesAndContents.useAreas.otherIndoorSpace + infoObject.surfacesAndContents.useAreas.buildingRelatedOutdoorSpace + infoObject.surfacesAndContents.useAreas.externalStorageSpace);
        ...
        eExtraInfo.append($("<li>")
            .text('pricePerSquareMeters: ' + pricePerSquareMeters));
        eExtraInfo.append($("<li>")
            .text('pricePerSquareMetersExtra: ' + pricePerSquareMetersExtra));
        eExtraInfo.append($("<li>")
            .text('pricePerQubicMeters: ' + pricePerQubicMeters));
        element.find('.object-header__pricing')
            .append(eExtraInfo);
    }
